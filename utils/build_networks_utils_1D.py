...
def conv_block_1D(in_filter, output_filter, nb_conv, kernel_size, stride, padding, final_nbchannels, normalize, wasserstein, layer_norm, spectral_norm, dropout, activation_function=nn.LeakyReLU(0.2, inplace=True)):
    """To simplify the creation of 1D convolutional sequences for discriminator
    ...
    for i in range(nb_conv):
        ...
        if layer_norm and spectral_norm: # No one used both of them at the same time -> Logical
            ...
            if spectral_norm:
                tmp_conv = nn.utils.spectral_norm(nn.Conv1d(nbchannel, nbfilter, kernel_size, stride, padding, bias=False))
            else:
                tmp_conv = nn.Conv1d(nbchannel, nbfilter, kernel_size, stride, padding, bias=False)
            ...
            if nbchannel != final_nbchannels:
                if normalize:
                    if layer_norm:
                        ...
                        sequential.append(nn.BatchNorm1d(nbfilter))
...
def deconv_block_1D(latent_vector_size, output_channels, nb_deconv, kernel_size, stride, padding, final_nbchannels, normalize, layer_norm, spectral_norm, batch_norm_proj, activation_function=nn.ReLU()):
    """To simplify the creation of fractionned strided 1D convolutional sequences for the generator
    ...
    for i in range(nb_deconv):
        ...
        if layer_norm and spectral_norm: # No one ever used both of them at the same time -> Logical
            ...
            if spectral_norm:
                tmp_deconv = nn.utils.spectral_norm(nn.ConvTranspose1d(nbchannel, nbfilter, kernel_size, stride, padding, bias=False))
            else:
                tmp_deconv = nn.ConvTranspose1d(nbchannel, nbfilter, kernel_size, stride, padding, bias=False)
...
def weights_init_1D(module, mean=0.0, std=0.02):
