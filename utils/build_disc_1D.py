...
class Discriminator_1D(nn.Module):
    def __init__(self, conv_sizes, scenario_shape, normalize, wasserstein, layer_norm, spectral_norm, dropout):
        """Constructor of DCGAN_like 1D discriminators

        Parameters
        ----------
        conv_sizes : int tuple list
            Each tuple describe a "convolution" block containing patterns of Conv1D, followed by a norm layer and an actvation one.
            ...
            ! Caution !
            This constructor is meant for DCGAN-like architectures in 1D, so if you want to use nb_blocks > 1 in a tuple, you
            ...
            Conv1D(latent_shape, 64, 3, 2, 1) -> Normalization(64) -> Activation -> Conv1D(64, 64, 3, 2, 1) -> Normalization(64) -> Activation ...
        scenario_shape : tensor size object
            Shape of the scenarios (time series) taken in input. It will be used mainly for the first layer
        ...
        conv_sizes = [(scenario_shape[0],)] + conv_sizes + [(1, 1, 5, 1, 0)]
        layers = it.chain(*[conv_block_1D(prev[0], curr[0], curr[1], curr[2], curr[3], curr[4], 1, normalize=normalize, wasserstein=wasserstein,
    ...
    def forward(self, scenario, with_pre_last=False):
